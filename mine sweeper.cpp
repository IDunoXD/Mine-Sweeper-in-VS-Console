...
char **gen_field(char **field,int a,int b,int bombs,boolean autoplay){
	...
	for(i=0;i<a;i++){
		for(j=0;j<b;j++){
			...
			if(rand()%((a*b)-(i*b)-j-used_space+skip)==0){ //гениальный рандом
				field[i][j]=Mine;
				bombs--;
				used_space++;
				skip=i=0;
	...
	/*cout<<endl;						//вывод поля на котором только мины(9)
	for(i=0;i<a;i++){
		for(j=0;j<b;j++){
			cout<<field[i][j]<<" ";
		}
	...
	/*cout<<endl;						//вывод поля на котором мины и цифры(мины числа > 8)
	for(i=0;i<a;i++){
		for(j=0;j<b;j++){
			cout<<field[i][j]<<" ";
		}
	...
	/*cout<<endl;						//вывод полноценного поля где мина(9)
	for(i=0;i<a;i++){
		for(j=0;j<b;j++){
			cout<<(int)field[i][j]<<" ";
		}
